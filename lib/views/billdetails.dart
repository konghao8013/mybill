...
class BillDetailPage extends StatefulWidget {
  DateTime _beginTime;
  DateTime _endTime;
  BillDetailPage(DateTime beginTime, DateTime endTime) {
    this._beginTime = beginTime;
    this._endTime = endTime;
  }
  @override
  _BillDetailState createState() => _BillDetailState(_beginTime, _endTime);
}
...
class _BillDetailState extends State<BillDetailPage> {
  ...
  _BillDetailState(DateTime beginTime, DateTime endTime) {
    this._beginTime = beginTime;
    this._endTime = endTime;
  }
  @override
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        body: Container(
          ...
          child: Column(
            children: billdetailList.map((item) {
              //根据备注是否有值决定展示
              var leftCol = new List<Widget>();
              leftCol.add(Text("${item.name}"));
              if (item.memo != null && item.memo.length > 0) {
                leftCol.add(Text("${item.memo}"));
              }
              ...
              var rel = Slidable(
                ...
                secondaryActions: <Widget>[
                  IconSlideAction(
                    ...
                    onTap: () async {
                      var bit =
                          await Msgbox.Confirm(context, "确认删除该笔账单吗？删除后无法恢复。",title: "警告");
                      if (bit) {
                      ...
                      Container(
                        child: Column(
                          ...
                          children: [
                            Text("￥${item.price}",
                                style: TextStyle(
                                  color: Color.fromRGBO(192, 56, 42, 1),
                                )),
                            Container(
                              margin: EdgeInsets.fromLTRB(0, 6, 0, 0),
                              child: Text(
                                  "${DateFormat('yyyy-MM-dd  kk:mm').format(item.createTime)}"),
                            )
                          ],
                        ),
                      )
